...
class SmsNotificationTemplatePreview(models.TransientModel):
    _name = "sms.notification.template.preview"
    _description = "SMS Notification Template Preview"
    ...
    def _get_records(self):
        ...
        if not sms_template_id:
            return []
        sms_template = self.env['sms.notification.template']
    ...
    def default_get(self, fields):
        result = super(SmsNotificationTemplatePreview, self).default_get(fields)
        sms_template = self.env['sms.notification.template']
